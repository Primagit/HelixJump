...
public class HelixController : MonoBehaviour {
    ...
    {
        Stage stage = allStages[Mathf.Clamp(stageNumber, 0, allStages.Count - 1)];//загрузка сцены (Stage 0)

        //устанока нового фона
        Camera.main.backgroundColor = allStages[stageNumber].stageBackgroundColor;
        FindObjectOfType<BallController>().GetComponent<Renderer>().material.color = allStages[stageNumber].stageBallColor;
        
        transform.localEulerAngles = startRotation;//обнулить вращение колонны
        ...
        for (int i = 0; i < stage.levels.Count; i++)
        {
            ...
            //деактивация случайных секторов платформ
            ...
            //создание deathPart-ов
            List<GameObject> leftParts = new List<GameObject>();

            foreach (Transform t in level.transform)
            {
                t.GetComponent<Renderer>().material.color = allStages[stageNumber].stageLevelPartColor; // установка цвета сектора
